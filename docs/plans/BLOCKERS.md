...
| **5** | **Barça Mobile — may the site show FC Barcelona's mark?** Client feedback of 21 September 2026 (item 12) removed Orange from the site entirely — Orange was never a TalentSync client, and the "was Orange a direct client or was NEVG the counterparty" question that used to sit here is answered: New Era Visionary Group is the counterparty, full stop. What is still open, per feedback item 13: **(a)** is there **written** permission to use the FC Barcelona / Barça Mobile **name and logo**; **(b)** is there a written source for *"1.5M downloads in first 3 months"* (still cut per D7 either way, unless (b) supplies one)? Same email: **the Moldova ICT headcount figure** — Moldova IT Park's published number with its year, or the sentence goes. | Permission to have *worked with* someone is not permission to *use their mark*, and clause A5.2 warrants that this consent is held. FC Barcelona licenses its brand commercially and is an aggressively enforced mark. **The site currently ships the Barça Mobile photo and the New Era Visionary Group logo on `/case-studies/` and on the homepage spotlight, unchanged from before this row was written** — that predates this ledger and was carried forward rather than gated, so (a) is not a theoretical risk, it is live. Pull both images to text-only pending a written answer, or get the written answer. An unsourced downloads figure is exactly the claim A4.3 promises to substantiate on request. | **V** |
